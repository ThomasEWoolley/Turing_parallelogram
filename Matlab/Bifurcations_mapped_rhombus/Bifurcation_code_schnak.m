...
theta=linspace(pi/50,pi/2,50);

theta=0.01;
parfor i=1:length(theta)
    the=theta(i);
    Fname=['Hom-',num2str(the)];
if exist(Fname, 'dir')
    rmdir([Fname,'*'],'s')
end
...
par=           [ 3   , 1  ,  10  ,  1/10   , 9/10,  the]; % Parameters used in functions and Jacobian
...
K=p.mat.K;
r=1/(L*sin(theta))^2*[Du*K 0*K; 0*K Dv*K]*[U;V]-p.mat.M*f; % putting rhs together
end
